...
You are given a list logs, where logs[i] represents the ith log message formatted
...
public:
    vector<int> exclusiveTime(int n, vector<string>& logs){
        ...
        for(const string& log : logs){
            stringstream ss(log);
            string f_id, status, timestamp;
            getline(ss, f_id, ':');
            getline(ss, status, ':');
            getline(ss, timestamp, ':');
            int curr_time = stoi(timestamp);
            if(status == "start"){
                if(!myStack.empty()){
                    result[myStack.top()] += curr_time - prev_time;
                }
                myStack.push(stoi(f_id));
                prev_time = curr_time;
            }
            else{
                result[myStack.top()] += curr_time - prev_time +1;
                myStack.pop();
